...
class ResNet18(nn.Module):
    def __init__(self, classes_num):
        ...
        self.prepare = nn.Sequential(           # 所有的ResNet共有的预处理
            nn.Conv2d(1, 64, 7, 2, 3),          # mnist为28*28*1单通道数据集，维持矩阵大小，1步长二维卷积=>64*28*28
            nn.BatchNorm2d(64),                 # 卷积后归一化处理，防止ReLU过大导致性能不稳定
            nn.ReLU(inplace=True),              # ReLU激活
            nn.MaxPool2d(3,2,1)               # 单步长最大池化，维持矩阵大小=>64*28*28
        )
        self.layer1 = nn.Sequential(            # 两个CommonBlock四次卷积，64*28*28=>64*28*28
            CommonBlock(64, 64, 1),
            CommonBlock(64, 64, 1)
        )
        ...
        self.layer3 = nn.Sequential(
            SpecialBlock(128, 256, [2, 1]),     # 升维下采样，Special+Common,128*28*28=>256*14*14
            CommonBlock(256, 256, 1)            # CommonBlock两次卷积，256*14*14=>256*14*14
        )
        self.layer4 = nn.Sequential(
            SpecialBlock(256, 512, [2, 1]),     # 升维下采样，Special+Common,256*28*28=>512*7*7
            CommonBlock(512, 512, 1)            # CommonBlock两次卷积，512*7*7=>512*7*7
        )
